...
let SEAM = 0, SYN = 0;   // SYN: with the 2007 models on, the world round the rectangle is made of the map's own pieces (synth07.js) and map07 answers there too
const seedAt = (x, z) => !M7 || (SEAM && !SYN && !g7In(x, z));
...
/* THE MADE WORLD'S REACH (synth07.js): how far past Gielinor's coast a place lies, 0..1 on a log scale that saturates SYN_FAR
   ...
   tile there, and the made squares run that far every way (map07's square ids outgrow the cache's byte for y). Nothing here
   touches the seed's own world. */
  ...
  if ((hash2(cx, cz, S + 21) % 1000) < 120 + 780 * smoothstep(-0.62, 0.55, civ)) {
    ...
    if (y > 1.9 && y < 34) {
      ...
      /* a made city's sprawl as it could grow (villageAt settles what it may): its core (r) keeps the table, the gates and the belt;
         ...
         for (a core is held to its cell; a sprawl is not), short of Gielinor's sea. How much room the wilds leave it is measured only
         for the cities that could be great (siteRoom) */
...
/* THE MADE WORLD'S GREAT CITIES. The world past the rectangle is cut two ways: into metro cells of 8 settlement cells a side (2000
   tiles), each holding at most one metropolis, and into town cells of 4 (1000 tiles), each holding at most one great town. A cell's
   city is its town of rank 2 or more with the widest sprawl (the luckier on a tie), grown to its tier's cap. It gives its sprawl
   up to a higher tier's city that would touch it, or to a stronger one of its own tier; any settlement a standing city's outline
   reaches is swallowed into it; every other town or city keeps its core size. Each answer is a few dozen cached lookups, and
   nothing can overlap by construction. */
const CITY_TIERS = [{ n: 8, max: 1260, k: 'm' }, { n: 4, max: 560, k: 'd' }];   // metropolises, then the great towns between them
const METRO_MAX = CITY_TIERS[0].max;
/* the room a city at s has: along twelve rays from its heart, 48 tiles a step, how far to the wilderness (the bands' edges wander
   too fast for wildD at the heart to say) and how far to open sea — neither the nearest ray, so a great city may be cut off by the
   ditch on a side or run along a coast, but most of it stands on open country */
function siteRoom(s) {
  if (s.room !== undefined) return s.room;
  const reach = Math.min(s.sprawl, METRO_MAX) * EXT_MAX, sea = [], wild = [];
  for (let k = 0; k < 12; k++) {
    const sx = Math.sin(k * TAU / 12), cz = Math.cos(k * TAU / 12);
    let wet = reach, ditch = reach;
    for (let r = 48; r < reach; r += 48) {
      const x = s.x + sx * r, z = s.z + cz * r;
      if (wildD(x, z) > -30) { ditch = r - 24; break; }   // past the ditch the ray is done: the city stops there on this side
      if (wet === reach && macroHeight(x, z) < -5) wet = r - 24;   // open sea or a deep lake: a river (never below -3.2) runs through a city
    }
    sea.push(Math.min(wet, ditch)); wild.push(ditch);
  }
  sea.sort((a, b) => a - b); wild.sort((a, b) => a - b);
  return s.room = Math.max(s.r, Math.floor(Math.min(wild[2], sea[4]) / EXT_MAX));   // two sides may meet the wilds (cityHolds clips the streets there), four the sea
}
const tierSprawl = (s, T) => Math.min(s.sprawl, T.max, s.room === undefined ? 1e9 : s.room);
const cityPri = (s, T) => tierSprawl(s, T) * 1024 + (s.h3 & 1023);
function tierCand(ti, mx, mz) {
  const T = CITY_TIERS[ti], key = T.k + 'c' + mx + ':' + mz;
  let c = nbrCache.get(key);
  if (c !== undefined) return c;
  const top = [];   // the three that could spread furthest, then whichever of them the wilds leave the most room
  for (let a = 0; a < T.n; a++) for (let b = 0; b < T.n; b++) {
    const s = siteInfo(mx * T.n + a, mz * T.n + b);
    if (s && s.sprawl > s.r && g7Out(s.x, s.z) >= 480) { top.push(s); top.sort((p, q) => Math.min(q.sprawl, T.max) - Math.min(p.sprawl, T.max)); if (top.length > 3) top.pop(); }
  }
  c = null;
  for (const s of top) { siteRoom(s); if (tierSprawl(s, T) > s.r && (!c || cityPri(s, T) > cityPri(c, T))) c = s; }
  nbrCache.set(key, c);
  return c;
}
function tierWin(ti, mx, mz) {   // the cell's great city, if nothing above it or stronger beside it would meet its outline
  const T = CITY_TIERS[ti], key = T.k + 'w' + mx + ':' + mz;
  let w = nbrCache.get(key);
  if (w !== undefined) return w;
  w = tierCand(ti, mx, mz);
  for (let u = 0; u < ti && w; u++) {   // a higher tier's standing city (the tiers above never ask the ones below)
    const U = CITY_TIERS[u], ux = Math.floor(w.cx / U.n), uz = Math.floor(w.cz / U.n);
    for (let a = -2; a <= 2 && w; a++) for (let b = -2; b <= 2 && w; b++) {
      const o = tierWin(u, ux + a, uz + b);
      if (o && (o === w || Math.hypot(o.x - w.x, o.z - w.z) < (tierSprawl(o, U) + tierSprawl(w, T)) * EXT_MAX + 40)) w = null;
    }
  }
  for (let a = -2; a <= 2 && w; a++) for (let b = -2; b <= 2 && w; b++) {   // a stronger one of its own tier, if that one stands: priorities only climb, so this ends
    if (!a && !b) continue;
    const o = tierCand(ti, mx + a, mz + b);
    if (o && cityPri(o, T) > cityPri(w, T) && Math.hypot(o.x - w.x, o.z - w.z) < (tierSprawl(o, T) + tierSprawl(w, T)) * EXT_MAX + 40 && tierWin(ti, mx + a, mz + b) === o) w = null;
  }
  nbrCache.set(key, w);
  return w;
}
function villageAt(cx, cz) {
  ...
  v = null;
  let tierOf = -1;
  if (s && M7 && SYN) {   // the made world: a great city sprawls, and anything its outline reaches is swallowed
    const own = s.rank >= 2 ? s.r * EXT_MAX : s.r * 1.8;
    for (let ti = 0; ti < CITY_TIERS.length; ti++) {
      const T = CITY_TIERS[ti], mx = Math.floor(cx / T.n), mz = Math.floor(cz / T.n);
      for (let a = -2; a <= 2; a++) for (let b = -2; b <= 2; b++) {
        const w = tierWin(ti, mx + a, mz + b);
        if (!w) continue;
        if (w === s) { if (tierOf < 0) tierOf = ti; continue; }
        if (Math.hypot(w.x - s.x, w.z - s.z) < tierSprawl(w, T) * EXT_MAX + own + 20) { villageCache.set(k, null); return null; }
      }
    }
  }
  if (s) {
    ...
    v.sprawl = tierOf >= 0 ? tierSprawl(s, CITY_TIERS[tierOf]) : s.sprawl ? s.r : 0;   // a made city: a great one sprawls to its tier's cap, the rest keep their core's size (synth07's plan either way)
    v.metro = tierOf + 1;   // 1 a metropolis, 2 a great town, 0 neither
  }
...
function citiesIn(x0, z0, x1, z1) {
  ...
  for (const T of CITY_TIERS) {
    const cell = T.n * SETTLE_CELL, R = T.max * EXT_MAX * 1.25 + 40;   // 1.25: the levelled ground eases out to 1.2 sprawls along the outline (synth07's syCityLevel)
    for (let mx = Math.floor((x0 - R) / cell); mx <= Math.floor((x1 + R) / cell); mx++)
      for (let mz = Math.floor((z0 - R) / cell); mz <= Math.floor((z1 + R) / cell); mz++) {
        const w = tierWin(CITY_TIERS.indexOf(T), mx, mz);
        if (!w) continue;
        const e = tierSprawl(w, T) * EXT_MAX * 1.25 + 8;
        if (w.x + e < x0 || w.x - e > x1 || w.z + e < z0 || w.z - e > z1) continue;
        const v = villageAt(w.cx, w.cz);
        if (v && v.metro && !out.includes(v)) out.push(v);
      }
  }
...
function biomeName(h, x, z) {
  if (M7 && (g7In(x, z) || !SEAM)) return m7Place(x, z);
...
function launch(tint, sx, sy, sz, o, dmg, speed, lo, hi, extra) {
  ...
  bolts.push(b);
  if (M7 && typeof c7FxBolt === 'function') c7FxBolt(b);   // the cache's cast, flight and splash graphics ride the bolt
  return b;
}
...
function boltFrame(dt) {
  for (let i = bolts.length - 1; i >= 0; i--) {
    ...
    if (b.g7) c7FxBoltFrame(b, dt, tx, tz);
    if (k >= 1) {
      b.m.visible = false; b.m.scale.setScalar(1);
      const g7end = b.fx7 ? c7FxBoltEnd(b, o) : false;
      if (!b.arw && !g7end) spellBurst(tx, ty, tz, b.sp.tint);
...
function refresh() {
  if (M7) {   // Gielinor streams whole map squares (46.); the seed's chunks come only where its world lies within reach
    m7Stream();
    if (!SEAM || SYN || g7Out(focus.x, focus.z) < -(RADIUS + 1) * CHUNK) { for (const [k, rec] of chunks) { disposeChunk(rec); chunks.delete(k); } pending.length = 0; popQueue.length = 0; return; }
  }
...
function osrsApply() {
  ...
  if (typeof synApply === 'function') synApply();   // the world past Gielinor's rectangle wears the same setting
...
const powerAt = (x, z) => M7 && (g7In(x, z) || !SEAM) ? m7Wild(x, z) / 26 : z > 500000 ? dunPower(x, z)
...
const wanderAt = (kh, tick, hx, hz, wr) => {   // wr: how far from home it strays (5 unless its spawn says otherwise; 0 stands still)
  const q = wr === undefined ? 5 : wr;
  if (q <= 0) return null;
  const r = hash2(kh ^ tick, tick, S + 777);
  return r % 6 === 0 ? { x: hx + ((r >>> 8) % (2 * q + 1)) - q, z: hz + ((r >>> 16) % (2 * q + 1)) - q } : null;
};
/* a watchman walks a beat: from his post to its far end and back, pausing at each, on a clock of his own */
function patrolAt(n) {
  const per = 40 + (n.kh >>> 3) % 32, t = tickN + (n.kh >>> 9) % per;
  if (t % per > 3) return null;   // a leg is set at its start, then walked
  const p = Math.floor(t / per) % 2 ? n.pat : n.home;
  return { x: p.x, z: p.z };
}
  ...
  if (n.target) {
   ...
   // The metal kinds alone hold their range (wiki), and a boss keeps to the phase it announced.
    ...
    if (near > want || !hasLos(n.tx, n.tz, P.tx, P.tz)) {   // out of reach, or behind something: close in until the line opens, as 2007 does
      ...
      if (n.fig && typeof c7Act === 'function') c7Act(n, 0);
   ...
   // unprotected it hits up to 50 as in 2007 (fmax overrides where the wiki differs, e.g. the KBD)
    ...
    const d = n.heldT > tickN || n.still ? null : n.pat ? patrolAt(n) : wanderAt(n.kh, tickN, n.home.x, n.home.z, n.wr);   // un-aggroed monsters mill about deterministically, unless rooted (or a Gielinor figure that never walks); a made world's watch walks its beat
    if (d) n.dest = d;
...
function killNpc(n) {
  ...
  if (M7 && n.fig && typeof c7Corpse === 'function') c7Corpse(n);   // the figure falls where it stood before the monster goes
  removeNpc(n);
...
function taskTick() {
  ...
  if (t.k === 'attack') {
   ...
   // each client owns its own hitpoints: we roll with our stats and send it; open ground ends a fight mid-chase
    ...
    if (dmg < 0) return;
    o.hp -= dmg;
    if (o.fig && o.hp > 0 && typeof c7Act === 'function') c7Act(o, 1);   // it flinches from the blow
...
function mapMarks() {
  ...
  if (M7 && typeof c7MiniIcons === 'function') c7MiniIcons(mctx, k, C, yaw + PI);
...
function animate(E, p, dt) {
  ...
  if (M7 && E === P && p.osrs && typeof c7Pose === 'function' && c7Pose(E, p, dt, moving)) return;   // the cache's own seqs on the 2007 kit (content07.js)
  if (p.a07own) OSRSK.rest07(p);
...
function frame(now) {
  ...
  mapTick();
  water.visible = !M7 || (SEAM && !SYN && g7Out(P.rx, P.rz) > -(scene.fog ? scene.fog.far : 400) - 40);   // deep inside the rectangle the seed's sea is past the fog: no plane at all
  ...
  if (M7 && scene.fog) {   // the map's squares stop at m7Reach; past the rectangle the view opens as far as the seed's own, never over squares not loaded
    const far = SEAM && !SYN ? clamp(g7Out(P.rx, P.rz), m7Reach(), OPT.viewRadius * CHUNK * 0.95) : m7Reach();
...
function applySave(b) {
  ...
  if (typeof c7Unpack === 'function') c7Unpack(b.g7);
...
function freshCharacter() {
  ...
  if (typeof c7Unpack === 'function') c7Unpack(null);
  dirty.inv = dirty.eq = dirty.sk = dirty.orb = 1;
}
  ...
  switch (m[0]) {
   ...
   // only the pid that actually holds it may let it go: element 8 is the relay's own record of the sender, not a claim
      ...
      if (act >= 1 && act <= 3) {
        ...
        if (q.fig && typeof c7Act === 'function') c7Act(q, 0);
...
function clueSpot(i, x, z) {   // dry, wild ground a tier's walk from (x, z)
  if (M7 && typeof c7ClueSpot === 'function') { const c = c7ClueSpot(i); if (c) return c; }   // Gielinor: the cache's own clue steps (content07b.js)
...
function readClue(i) {   // a scroll without a spot (an old save) is given one from where you stand
  if (M7 && typeof c7ReadClue === 'function') return c7ReadClue(i);
...
function dig() {
  if (M7 && typeof c7ClueDig === 'function' && c7ClueDig()) return;
  const c = P.clue;
  kneel(); sfx(1470);
...
function skillGuide(i) {
  if (M7 && typeof c7Guide === 'function' && !skillGuide.own) { c7Guide(i).then(ok => { if (!ok) { skillGuide.own = 1; skillGuide(i); skillGuide.own = 0; } }); return; }   // Gielinor reads the client's own guide
...
function m7Spawn(s, key) {
  ...
  scene.add(mesh);
  let dest = null;
  for (let b = 0; b < 12 && !dest && !still && !s.pat; b++) dest = wanderAt(kh, tickN - b, x, z, wr);   // adopt the leg the room is already walking
  const n = { npc: 1, t, key, name: t.n, tx: x, tz: z, px: x, pz: z, rx: x, rz: z, ry: m7Y(x, z, pl), home: { x, z }, hp: t.hp, maxhp: t.hp, face: (kh & 3) * PI / 2, faceT: (kh & 3) * PI / 2,
    ...
    wr, pat: s.pat ? { x: s.pat[0] + fp, z: -(s.pat[1] + fp) } : null };
  ...
  MAP07.npcFigure(def).then(fig => {
    ...
    if (typeof c7WarmNpc === 'function') c7WarmNpc(n);   // its attack, flinch and death frames load before the first blow
    mesh.add(fig.mesh);
...
function m7NpcOpts(n) {
  ...
  for (const op of ops) {
    ...
    else if (low === 'talk-to') out.push({ t: 'Talk-to', o: name, f: ui(() => typeof c7TalkTo === 'function' ? c7TalkTo(n) : say(name + ': ' + (M7_SLAYER[nm] !== undefined ? 'Need a task? Ask me for an assignment.' : M7_CHAT[n.kh % M7_CHAT.length]))) });
    else out.push({ t: op, o: name, f: ui(() => { if (!(typeof c7NpcOp === 'function' && c7NpcOp(n, op))) say('Nothing interesting happens.'); }) });
  }
...
function m7LocOpts(o) {
  ...
  for (const op of ud.ops) {
    ...
    else if (typeof c7LocOp === 'function' && c7LocOp(ud, op, o, out)) continue;   // quests, clues, fairy rings, the farm's verbs (content07b.js)
    else if (/^(open|unlock|pick-lock)$/.test(low)) out.push({ t: op, o: ud.name, f: act(o, () => say(/gate|door|grill|bars|barrier/i.test(ud.name) ? 'The ' + ud.name.toLowerCase() + ' is locked.' : 'It\'s locked.')) });
...
function m7Door(ud, op) {   // both leaves of a double door swing together; the state is this client's own (a door never blocks)
  ...
  MAP07.toggleDoor(ud, partner);
  if (partner) MAP07.toggleDoor(partner, ud);
...
function m7Frame(dt) {
  ...
  if (typeof c7Frame === 'function') c7Frame(dt);   // animated scenery, falling figures, spell graphics, the content systems' clocks
}
...
function m7MapRow(data, j, ox, z) {
  ...
  for (let i = 0; i < MW; i++) {
    ...
    if (SEAM && !SYN && !g7In(gx, -gy)) { const c = seedTileRGB(gx, -gy); data[p] = c >> 16 & 255; data[p + 1] = c >> 8 & 255; data[p + 2] = c & 255; data[p + 3] = 255; continue; }   // the seed's world past the rectangle
    let c = MAP07.tileRGB(P.plane, gx, gy), k = 1;
    if (c >= 0) { const wb = MAP07.wallBits(P.plane, gx, gy); if (wb & 15) c = wb & 16 ? 0xc8321e : 0xeeeeee; }
    else if (SYN && SEAM && !g7In(gx, -gy)) { c = syTileRGB(gx, gy); k = 0.8; }   // a made square not yet up: its ground's own colour, dimmed as the map's unloaded squares are
...
function m7Ready() {
  ...
  if (typeof c7Warm === 'function') c7Warm();
  if (OPT.osrs && typeof syLoad === 'function') syLoad().catch(() => {});   // the made world's palettes, ahead of the first square past the edge
  return MAP07.load();
}
function m7Mode(on) {
  on = on ? 1 : 0;
...
/* the world past the rectangle is made of the map's own pieces while the 2007 models are on (synth07.js, section 50): the maker
   feeds map07 its squares and the seed's chunks, spawns and objects stand down there; with the models off the seed world answers
   as before. The palettes load once, on the way into Gielinor */
function synApply() {
  const want = M7 && OPT.osrs && typeof syProvider !== 'undefined' ? 1 : 0;
  if (want && !SY.data) syLoad().then(() => synApply(), e => console.warn('[seedworld] the world past the map could not load', e && e.message));
  const on = want && SY.data ? 1 : 0;
  if (m7Inited && MAP07.ready()) MAP07.setSynth(on ? syProvider : null);
  if (on === SYN) return;
  SYN = on;
...
function osMapTiles() {   // the picture under the minimap: tiles square round the player (80 at the client's zoom, more zoomed out), four pixels a tile, north up
  ...
  for (let j = 0; j < N; j++) for (let i = 0; i < N; i++) {
    ...
    if (c < 0) { const sk = tk(gx, -gy); c = !SEAM || g7In(gx, -gy) ? 0 : SYN ? syTileRGB(gx & ~1, gy & ~1) : seedRGB.has(sk) ? seedRGB.get(sk) : seedTileRGB(gx & ~1, -(gy & ~1)); }   // past the rectangle the seed's world: the tile the round minimap already painted, else two tiles a sample until it has
  ...
  g.putImageData(im, 0, 0);
  if (typeof c7MapScenes === 'function') c7MapScenes(g, x0, yTop, N);   // the trees, rocks and fences the client draws on its minimap
...
function osEmotes() {
  ...
  Promise.all([OSUI.enumOf(1000), OSUI.enumOf(1001), OSUI.enumOf(1002)]).then(([names, spr, locked]) => {
    Object.keys(names).sort((a, b) => a - b).forEach((k, n) => {
      ...
      on(g, 'click', () => open && typeof c7Emote === 'function' ? c7Emote(k, names[k]) : say(open ? 'You can\'t perform emotes in this world yet.' : 'You haven\'t unlocked this emote yet.'));
    });
...
function osMusic() {
  ...
  OSUI.cfg('dbtableindex', 44).then(ix => {
    ...
    const tracks = rows.filter(r => r && r[1] && !(r[1][9] && r[1][9][0])).map(([id, v]) => ({ id, sort: String(v[0] && v[0][0] || ''), n: String(v[1] && v[1][0] || ''), open: typeof c7MusicOpen === 'function' && M7 ? c7MusicOpen(String(v[1] && v[1][0] || ''), !!(v[6] && v[6][0]) || !v[5]) : !!(v[6] && v[6][0]) || !v[5] }))
